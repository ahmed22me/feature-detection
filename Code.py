...
def NonMaximalSuppression(img, radius):
    ...
    radius_height=abs(radius-height)
    radius_width=abs(radius-width)
    
    img_out = np.zeros((height, width))

    for y in range(radius_height):
        for x in range(radius_width):
